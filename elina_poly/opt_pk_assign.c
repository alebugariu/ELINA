...
opt_numint_t* ntab, elina_dim_t dim,
opt_numint_t* tab, size_t size) {
	size_t i;
...
opt_pk_array_t* opt_poly_asssub_linexpr_det(bool assign, elina_manager_t* man,
		bool destructive, opt_pk_array_t* oa, elina_dim_t dim,
	...
	if (!contains_comp(clb, var)) {
		//if(cli!=NULL && is_disjoint(cli,clb,maxcols)){
		//	need_refine = true;
		//}
		insert_comp(clb, var);
	}
	...
	opt_vector_set_elina_linexpr0(opk, opk->poly_numintp, dst, comp_size, 1);
	...
	if (flag1) {
		...
		matF = opt_matrix_alloc(nbvertex + nbline + 1, poly[res]->intdim + 2,
				false);
	...
	if (flag1 || flag2) {
		fuse_generators_intersecting_blocks(matF, poly_a, acla, ca,
				num_vertex_a, disjoint_map, maxcols);
		...
		for (k = 0; k < num_compa; k++) {
			if (rmapa[k] == res) {
				...
				for (i = 0; i < nbconsa; i++) {
					...
					if (opt_vector_is_positivity_constraint(opk, sai,
							src->nbcolumns)) {
						flag = true;
					}
					dai[0] = sai[0];
	...
	if (!sgn) { /* Expression is not invertible */
		...
		if (assign) {
			...
			poly[res]->F = opt_matrix_alloc(matF->nbrows, matF->nbcolumns,
					false);
			poly[res]->nbline = opt_matrix_assign_variable(opk, poly[res]->F,
					matF, nvar, opk->poly_numintp);
			if (opk->exn) {
			...
			elina_dim_t tdim[1];
			tdim[0] = nvar;
			opt_poly_projectforget_array(false, man, poly[res], poly[res],
					&tdim, 1, true);
			...
			poly[res]->satC = opt_satmat_alloc(poly[res]->F->nbrows,
					opt_bitindex_size(poly[res]->C->nbrows));
			combine_satmat(opk, poly[res], matC->nbcolumns - opk->dec,
					poly[res]->C->nbrows, true);

			opt_matrix_free(matF);
		} else {
			poly[res]->C = opt_matrix_substitute_variable(opk, true, matC, nvar,
					opk->poly_numintp);
		...
		opt_vector_invert_expr(opk, opk->poly_numintp2, nvar, opk->poly_numintp,
	...
	for (k = 0; k < num_compa; k++) {
		...
		if (destructive) {
			...
			poly[ind]->satF =
					poly_a[k]->satF ? opt_satmat_copy(poly_a[k]->satF) : NULL;
			poly[ind]->satC =
					poly_a[k]->satC ? opt_satmat_copy(poly_a[k]->satC) : NULL;
			poly[ind]->nbline = poly_a[k]->nbline;
		}
	}
	//cl = cl->next;
	...
	opt_poly_asssub_linexpr_det_exit: if (destructive) {
		for (k = 0; k < num_compa; k++) {
			if (!disjoint_map[k]) {
...
/* ====================================================================== */
static opt_pk_array_t* opt_poly_asssub_linexpr(bool assign, bool lazy,
		elina_manager_t* man, bool destructive, opt_pk_array_t* oa,
		elina_dim_t dim, elina_linexpr0_t* linexpr, opt_pk_array_t* ob) {
	...
	if (elina_linexpr0_is_linear(linexpr)) {
		op = opt_poly_asssub_linexpr_det(assign, man, destructive, oa, dim,
				linexpr);
		...
		op = elina_generic_asssub_linexpr_array(assign, man, destructive, oa,
				&dim, &linexpr, 1, ob);
	}
...
opt_pk_array_t* opt_poly_asssub_linexpr_array_det(bool assign,
		elina_manager_t* man, bool destructive, opt_pk_array_t* oa,
	...
	array_comp_list_t * aclb = create_array_comp_list();
	comp_list_t ** clb_arr = (comp_list_t **) malloc(
			size * sizeof(comp_list_t *));
	for (i = 0; i < size; i++) {
	...
	elina_linexpr0_t ** expr_array = (elina_linexpr0_t **) malloc(
			size * sizeof(elina_linexpr0_t *));
	for (i = 0; i < size; i++) {
		unsigned short int ind = is_comp_list_included(acl, clb_arr[i],
				maxcols);
		rmapb[i] = ind;
		expr_array[i] = copy_linexpr0_with_comp_list(opk, texpr[i], ca_arr[ind],
				comp_size_map[ind]);
		nbmapb[ind]++;
	}
	...
	for (k = 0; k < num_comp; k++) {
		...
		if (!nbmapb[k]) {
			...
			if (destructive) {
				...
				poly[k]->C =
						poly_a[l]->C ? opt_matrix_copy(poly_a[l]->C) : NULL;
				poly[k]->nbeq = poly_a[l]->nbeq;
				poly[k]->F =
			...
			if (assign) {
				...
				poly[k]->C = opt_matrix_alloc(nbgenmapa[k] + 1, comp_size + 2,
						false);
	...
	if (assign) {
		...
		cartesian_product_vertices_with_map(oa, poly, rmapa, ca_arr,
				num_vertex_a, num_vertex, counterF, disjoint_map);
		...
		meet_cons_with_map(opk, oa, poly, rmapa, ca_arr, counterF, pos_con_map,
				disjoint_map);
		for (k = 0; k < num_comp; k++) {
		...
		if (nbmapb[k]) {
			...
			for (i = 0; i < size; i++) {
				if (rmapb[i] == k) {
					...
					opt_vector_set_elina_linexpr0(opk, tvec[l], expr_array[i],
							comp_size, 1);
			...
			if (assign) {
				poly[k]->F = opt_matrix_assign_variables(opk, oak->F, tdim2,
						tvec, nbmapb[k]);
				if (opk->exn) {
					if (opk->exn != ELINA_EXC_OVERFLOW) {
						opk->exn = ELINA_EXC_NONE;
				...
				poly[k]->C = opt_matrix_substitute_variables(opk, oak->C, tdim2,
						tvec, nbmapb[k]);
			}
			opt_matrix_free(tmp);
...
/* ====================================================================== */
static opt_pk_array_t* opt_poly_asssub_linexpr_array(bool assign, bool lazy,
		elina_manager_t* man, bool destructive, opt_pk_array_t* oa,
		elina_dim_t* tdim, elina_linexpr0_t** texpr, size_t size,
		opt_pk_array_t* ob) {
	opt_pk_array_t* op;
	...
	if (elina_linexpr0_array_is_linear(texpr, size)) {
		op = opt_poly_asssub_linexpr_array_det(assign, man, destructive, oa,
				tdim, texpr, size);
		if (ob) {
			opt_poly_meet(true, lazy, man, op, op, ob);
		}
...
/* ====================================================================== */
opt_pk_array_t* opt_poly_asssub_texpr_array(bool assign, bool lazy,
		elina_manager_t* man, bool destructive, opt_pk_array_t* oa,
	...
	if (elina_texpr0_array_is_scalar(texpr, size)
		...
		if (size == 1) {
			elina_linexpr0_t* linexpr0 = elina_intlinearize_texpr0(man, &abs,
					texpr[0], NULL, ELINA_SCALAR_MPQ, false);
			op = opt_poly_asssub_linexpr_det(assign, man, destructive, oa,
					tdim[0], linexpr0);
			elina_linexpr0_free(linexpr0);

		} else {
			elina_linexpr0_t** tlinexpr = elina_intlinearize_texpr0_array(man,
					&abs, texpr, size, NULL, ELINA_SCALAR_MPQ, false);
			op = opt_poly_asssub_linexpr_array_det(assign, man, destructive, oa,
					tdim, tlinexpr, size);
		...
		op = elina_generic_asssub_texpr_array(assign, man, destructive, oa,
				tdim, texpr, size, ob);
	}
	...
	/* Is the result exact or best ? */
	if (opk->funopt->flag_best_wanted || opk->funopt->flag_exact_wanted) {
		man->result.flag_best = elina_texpr0_array_is_interval_linear(texpr,
				size);
		man->result.flag_exact = man->result.flag_best;
	} else {
		man->result.flag_best = man->result.flag_exact = false;
...
opt_pk_array_t* opt_pk_assign_linexpr_array(elina_manager_t* man,
		bool destructive, opt_pk_array_t* oa, elina_dim_t* tdim,
		elina_linexpr0_t** texpr, size_t size, opt_pk_array_t* ob) {
	...
	op = size == 1 ?
			opt_poly_asssub_linexpr(true, opk->funopt->algorithm <= 0, man,
					destructive, oa, tdim[0], texpr[0], ob) :
...
#endif
	op = size == 1 ?
			opt_poly_asssub_linexpr(false, opk->funopt->algorithm <= 0, man,
					destructive, oa, tdim[0], texpr[0], ob) :
			opt_poly_asssub_linexpr_array(false, opk->funopt->algorithm <= 0,
					man, destructive, oa, tdim, texpr, size, ob);
#if defined(TIMING)
...
opt_pk_array_t* opt_pk_assign_texpr_array(elina_manager_t* man,
		bool destructive, opt_pk_array_t* oa, elina_dim_t* tdim,
		elina_texpr0_t** texpr, size_t size, opt_pk_array_t* dest) {
	...
	start_timing();
#endif
	op = opt_poly_asssub_texpr_array(true, opk->funopt->algorithm <= 0, man,
			destructive, oa, tdim, texpr, size, dest);
#if defined(TIMING)
	record_timing(assign_linexpr_time);
#endif
	return op;
