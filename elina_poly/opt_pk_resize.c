...
opt_pk_array_t* opt_pk_add_dimensions_cons(elina_manager_t* man,
		bool destructive, opt_pk_array_t* oa, elina_dimchange_t* dimchange,
		bool project) {
	opt_pk_array_t* op;
	opt_pk_internal_t* opk = opt_pk_init_from_manager(man,
			ELINA_FUNID_ADD_DIMENSIONS);
	opt_pk_internal_realloc_lazy(opk,
			oa->maxcols + dimchange->intdim + dimchange->realdim - 2);
	...
	if (oa->is_bottom || !acla) {
		...
		if (destructive) {
			...
			return opt_pk_bottom(man, oa->maxcols + size - opk->dec, 0);
		}
	}
			...
			if (!oak->C && !oak->F) {
				...
				if (destructive) {
					...
					return opt_pk_bottom(man,
							oa->maxcols + dimchange->intdim - 2, 0);
				}
			}
		}
	}
	...
	unsigned short int * cmap = (unsigned short int *) calloc(maxcols + 1,
			sizeof(unsigned short int));
	unsigned short int * ncmap = (unsigned short int *) calloc(size,
			sizeof(unsigned short int));
	l = 0;
	k = opk->dec;
...
/***********************************
 Add dimensions
 ************************************/
opt_pk_array_t* opt_pk_add_dimensions(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, elina_dimchange_t* dimchange, bool project) {
#if defined (TIMING)
	start_timing();
#endif
...
/*******************************
 Remove Dimensions
 ********************************/
opt_pk_array_t* opt_pk_remove_dimensions(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, elina_dimchange_t* dimchange) {
	...
	if (oa->is_bottom || !acla) {
		...
		if (destructive) {
			...
			record_timing(remove_dimension_time);
#endif
			return opt_pk_bottom(man, oa->maxcols - dimsup - opk->dec,
					dimchange->realdim);
		}
		...
		if (opk->exn) {
			...
			if (!oak->F) {
				...
				opt_pk_array_t * op =
						destructive ?
								oa :
								opt_pk_array_alloc(NULL, NULL, oa->maxcols);
				op->maxcols -= dimsup;
	...
	/*********************************
	 Handle independent components
	 *********************************/
	unsigned short int * map = (unsigned short int *) calloc(maxcols,
			sizeof(unsigned short int));
	...
	if (destructive) {
		...
		while (k < num_compa) {
			 ...
			 ****************************/
			elina_dim_t * ndim = (elina_dim_t *) calloc(comp_size,
					sizeof(elina_dim_t));
			...
			if (size == comp_size) {
				...
				poly_a[k]->F = opt_matrix_remove_dimensions(opk, false, oak->F,
						&dimchange1);
		...
		for (k = 0; k < num_compa; k++) {
			...
			if (size == comp_size) {
				...
				dimchange1.intdim = size;
				dimchange1.realdim = 0;
				poly[k1] = opt_poly_alloc(oak->intdim - size, oak->realdim);
				poly[k1]->F = opt_matrix_remove_dimensions(opk, false, oak->F,
						&dimchange1);
				poly[k1]->nbeq = 0;
...
opt_pk_array_t* opt_pk_permute_dimensions(elina_manager_t* man,
		bool destructive, opt_pk_array_t* oa, elina_dimperm_t* permutation) {
#if defined(TIMING)
	start_timing();
#endif
	opt_pk_array_t* op;
	...
	unsigned short int **nca_arr = (unsigned short int **) calloc(num_comp,
			sizeof(unsigned short int **));
	unsigned short int maxcols = oa->maxcols;
	/**********************
	...
	opt_pk_t ** poly;
	poly = destructive ?
			poly_a : (opt_pk_t **) malloc(num_comp * sizeof(opt_pk_t *));
	if (!destructive) {
		for (k = 0; k < num_comp; k++) {
			opt_pk_t * src = poly_a[k];
	...
	for (k = 0; k < num_comp; k++) {
		...
		opt_pk_t *oak = poly_a[k];
		man->result.flag_best = man->result.flag_exact = true;
		elina_dim_t * dim = (elina_dim_t *) calloc(comp_size,
				sizeof(elina_dim_t));
		unsigned short int i, j;
		for (i = 0; i < comp_size; i++) {
		...
		if (oak->C) {
			poly[k2]->C = opt_matrix_permute_dimensions(opk, destructive,
					oak->C, dim);
		}
		if (oak->F) {
			poly[k2]->F = opt_matrix_permute_dimensions(opk, destructive,
					oak->F, dim);
		}
		cla = cla->next;
		free(ca);
