...
static void opt_esatmat_isort_rows(opt_satmat_row_t* tab, opt_satmat_t* sat) {
	...
	for (i = 1; i < sat->nbrows; i++) {
		...
		j = i;
		while (j > 0
				&& opt_bitstring_cmp(tab[j - 1].p, row.p, sat->nbcolumns) > 0) {
			tab[j] = tab[j - 1];
			j--;
		}
		tab[j] = row;
...
static int opt_qsort_rows_compar(void* qsort_man, const void* p1,
		const void* p2) {
...
static void opt_esatmat_sort_rows(opt_satmat_row_t* tab, opt_satmat_t* sat) {
	if (sat->nbrows >= 6) {
		...
		opt_qsort2(tab, (size_t) sat->nbrows, sizeof(opt_satmat_row_t),
...
int opt_esatmat_index_in_sorted_rows(opt_bitstring_t* satline,
		opt_satmat_row_t* tab, opt_satmat_t* sat) {
...
opt_numint_t *v2, unsigned short int *ca1, unsigned short int *ca2,
		unsigned short int comp_size1, unsigned short int comp_size2) {
	bool res;
...
opt_numint_t * v2, unsigned short int * ca1, unsigned short int * ca2,
	...
	return is_vectors_coeff_equal_comp_list(opk, v1, v2, ca1, ca2, comp_size1,
			comp_size2);
}

void vector_copy_comp_list(opt_pk_internal_t *opk, opt_numint_t * dst,
		opt_numint_t * src, unsigned short int * ind_map,
...
opt_numint_t *cons, opt_bitstring_t * satline) {
	...
	while (ind.index < mat->nbrows) {
		opt_numint_t prod = opt_vector_product(opk, mat->p[ind.index], cons,
				nbcolumns);
		if (prod) {
...
comp_list_t * vector_to_comp_list(opt_pk_internal_t *opk, opt_numint_t *v,
...
void opt_poly_widening_gen(elina_manager_t *man, opt_pk_array_t **res,
	...
	unsigned short int * comp_size_arr = (unsigned short int *) calloc(num_comp,
			sizeof(unsigned short int));
	...
	for (k = 0; k < num_comp; k++) {
		...
		tmp[k]->F = opt_matrix_alloc(nbgena[k] + 2 * num_vertex[k],
				comp_size + 2, false);
		poly[k]->C = opt_matrix_alloc(opk->dec - 1 + nbmapb[k], comp_size + 2,
				false);
		poly[k]->F = NULL;
	...
	cartesian_product_vertices(oa, tmp, rmapa, ca_arr, num_vertex_a, num_vertex,
			counterF);
	...
	for (k = 0; k < num_comp; k++) {
		...
		tmp[k]->satF = opt_satmat_alloc(C->nbrows,
	...
	for (kb = 0; kb < num_compb; kb++) {
		...
		for (i = 0; i < nbcons; i++) {
			...
			opt_numint_t * npci = opt_map_vector(pci, ind_map_b,
			...
			if (index >= 0) {
				...
				opt_vector_copy(poly[indb]->C->p[i1], npci,
						comp_size_arr[indb] + 2);
				counter[indb]++;
				//}
			}
			free(npci);
...
opt_pk_array_t* opt_pk_widening(elina_manager_t* man, opt_pk_array_t* oa,
		opt_pk_array_t* ob) {
#if defined(TIMING)
	start_timing();
#endif
