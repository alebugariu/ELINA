...
/* Expand the dimension dim of the matrix into (dimsup+1)
 ...
 offset. */
static opt_matrix_t* opt_matrix_expand(opt_pk_internal_t* opk, bool destructive,
		opt_matrix_t* C, elina_dim_t dim, size_t offset, size_t dimsup) {
	...
	for (i = 0; i < nbrows; i++) {
		if (p[i][col] != 0) {
			for (j = offset; j < offset + dimsup; j++) {
				...
				opt_vector_copy(p[row], p[i], nbcols + dimsup);
...
opt_pk_array_t* opt_pk_expand(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, elina_dim_t dim, size_t dimsup) {
	...
	opt_pk_t ** poly =
			destructive ?
					poly_a :
					(opt_pk_t **) malloc(num_compa * sizeof(opt_pk_t *));
	...
	for (k = 0; k < num_compa; k++) {
		if (k == ind) {
			...
			if (destructive) {
				...
				oak->nbeq = 0;
				oak->status &= ~opt_pk_status_consgauss
						& ~opt_pk_status_gengauss & ~opt_pk_status_minimaleps;
			} else {
			...
			free(ca);
			poly[k]->C = opt_matrix_expand(opk, destructive, oak->C, ndim,
					cl->size - dimsup, dimsup);
			/* Minimize the result */
			if (opk->funopt->algorithm > 0) {
...
void opt_pk_fold_new_comp(opt_pk_internal_t *opk, bool destructive,
		opt_pk_t ** poly, array_comp_list_t *acl, elina_dim_t *tdim,
		size_t size) {
	size_t i;
...
static opt_matrix_t* opt_matrix_fold_same_comp(opt_pk_internal_t* opk,
		bool destructive, opt_matrix_t* F, elina_dim_t dim, elina_dim_t* tdim,
		size_t size) {
	opt_matrix_t* nF;
	size_t i, j, row, col;
	...
	nF = opt_matrix_alloc(size * nbrows + nbrows, nbcols - dimsup, false);
	dimchange = elina_dimchange_alloc(dimsup, 0);
	for (i = 0; i < dimsup; i++) {
...
static opt_matrix_t* opt_matrix_fold_diff_comp(opt_pk_internal_t* opk,
		bool destructive, opt_matrix_t* F, opt_matrix_t ** fold_val,
	...
	nF = opt_matrix_alloc(2 * nbrows * fold_size + nbrows, nbcols, false);
	...
	for (i = 0; i < nbrows; i++) {
		...
		for (j = 0; j < dimsup; j++) {
			...
			for (i1 = 0; i1 < fv->nbrows; i1++) {
				...
				bool cond2 = (is_line(pi) && is_line(si))
						|| (is_ray(pi) && is_ray(si));
...
opt_pk_array_t* opt_pk_fold(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, elina_dim_t* tdim, size_t size) {
	...
	free(fca);
	opt_matrix_t ** fold_val = (opt_matrix_t **) malloc(
			dimsup * sizeof(opt_matrix_t *));
	for (k = 0; k < dimsup; k++) {
		fold_val[k] = NULL;
	}
	...
	while (k < num_compa) {
		...
		unsigned short int comp_size = cl->size;
		elina_dim_t * tdimk = (elina_dim_t *) calloc(comp_size,
				sizeof(elina_dim_t));
		...
		if (!flag) {
			...
			if (cl == fcl) {
				if (destructive) {
					poly_a[k]->F = opt_matrix_fold_same_comp(opk, true,
							poly_a[k]->F, ndim, tdimk, dim_size);
				} else {
					poly[k2] = opt_poly_alloc(oak->intdim - dim_size,
							oak->realdim);
					poly[k2]->F = opt_matrix_fold_same_comp(opk, false,
							poly_a[k]->F, ndim, tdimk, dim_size);
				...
				if (destructive) {
					...
					poly_a[k]->F = opt_matrix_remove_dimensions(opk, false,
							oak->F, &dimchange1);
					...
					poly[k2] = opt_poly_alloc(oak->intdim - dim_size,
							oak->realdim);
					poly[k2]->F = opt_matrix_remove_dimensions(opk, false,
							oak->F, &dimchange1);
					poly[k2]->nbeq = 0;
	...
	if (!flag1) {
		...
		if (destructive) {
			...
			src->nbeq = 0;
			src->status &= ~opt_pk_status_consgauss & ~opt_pk_status_gengauss
					& ~opt_pk_status_minimaleps;
		}
		opt_matrix_t * tmp2 = poly[ind]->F;

		poly[ind]->F = opt_matrix_fold_diff_comp(opk, destructive, src->F,
				fold_val, ndim, size);
		if (!destructive) {
