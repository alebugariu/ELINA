...
opt_pk_array_t * opt_pk_array_alloc(opt_pk_t ** poly, array_comp_list_t *acl,
...
opt_pk_array_t* opt_pk_copy(elina_manager_t* man, opt_pk_array_t* src) {
	...
	for (k = 0; k < num_comp; k++) {
		poly[k] = opt_poly_alloc(spoly[num_comp - k - 1]->intdim,
				spoly[num_comp - k - 1]->realdim);
		poly[k]->C =
				spoly[num_comp - k - 1]->C ?
						opt_matrix_copy(spoly[num_comp - k - 1]->C) : NULL;
		poly[k]->F =
				spoly[num_comp - k - 1]->F ?
						opt_matrix_copy(spoly[num_comp - k - 1]->F) : NULL;
		poly[k]->satC =
				spoly[num_comp - k - 1]->satC ?
						opt_satmat_copy(spoly[num_comp - k - 1]->satC) : NULL;
		poly[k]->satF =
				spoly[num_comp - k - 1]->satF ?
						opt_satmat_copy(spoly[num_comp - k - 1]->satF) : NULL;
...
void opt_poly_chernikova(elina_manager_t* man, opt_pk_t* poly, char* msg) {
	...
	if ((poly->C && poly->F) || (!poly->C && !poly->F)) {
		...
		if (poly->C) {
			...
			opt_matrix_normalize_constraint(opk, poly->C, poly->intdim,
					poly->realdim);
			//}
	...
	return;
	poly_chernikova_exit0: poly->status = 0;
	{
		char str[160];
		sprintf(str, "conversion from %s %s\n",
				poly->C ?
						"constraints to generators" :
						"generators to constraints", msg);
 ...
 ********************/
char meet_cons_one_comp(opt_pk_internal_t *opk, opt_pk_t **poly_a,
		array_comp_list_t *acla, unsigned short int **ca_arr, opt_pk_t *poly,
		unsigned short int * ca, char * map) {
	...
	for (k = 0; k < num_compa; k++) {
		...
		for (i = 0; i < nbconsa; i++) {
			...
			if (opt_vector_is_positivity_constraint(opk, src_pi,
					comp_size + 2)) {
				//map[ind] = 1;
				flag = true;
				continue;
 ...
 ********************/
void meet_cons_with_map(opt_pk_internal_t *opk, opt_pk_array_t *oa,
		opt_pk_t **poly, unsigned short int *rmapa, unsigned short int **ca_arr,
		size_t *counterC, char *map, char * exclusion_map) {
...
void meet_cons(opt_pk_internal_t *opk, opt_pk_array_t *oa, opt_pk_t **poly,
		unsigned short int *rmapa, unsigned short int **ca_arr,
		size_t *counterC, char *map) {
	array_comp_list_t *acla = oa->acl;
	unsigned short int num_compa = acla->size;
	unsigned short int maxcols = oa->maxcols;
	unsigned short int j, k;
...
size_t cartesian_product_vertices_one_comp(opt_pk_t **poly_a,
		array_comp_list_t *acla, unsigned short int ** ca_arr,
		size_t * num_vertex_a, opt_pk_t * poly, size_t counter,
		unsigned short int *ca, char * map) {
...
void meet_rays_one_comp(opt_pk_t **poly_a, array_comp_list_t *acla,
		unsigned short int **ca_arr, size_t * nblinemap, opt_pk_t *poly,
		size_t * num_vertex_a, unsigned short int * ca, size_t start,
		char * map) {
	/************************
	 Consider rays of A
	 ************************/
	...
	for (k = 0; k < num_compa; k++) {
		...
		size_t nbconsa =
				nblinemap == NULL ?
						src_mat->nbrows : src_mat->nbrows - nblinemap[k];
		opt_numint_t ** src_p = src_mat->p;
		opt_numint_t ** dst_p = dst_mat->p;
...
void meet_rays_with_map(opt_pk_array_t *oa, opt_pk_t **poly,
		unsigned short int *rmapa, unsigned short int **ca_arr,
		size_t * num_vertex_a, size_t * counterF, char * exclusion_map) {
	/************************
	 Consider rays of A
	 ************************/
	opt_pk_t **poly_a = oa->poly;
...
void combine_satmat(opt_pk_internal_t *opk, opt_pk_t *poly,
		unsigned short int comp_size, size_t end, bool con_to_ray) {
	size_t i, j;
...
void opt_pk_array_fprint(FILE* stream, elina_manager_t * man,
		opt_pk_array_t * oa, char ** name_of_dim) {
