...
int main(int argc, char **argv) {
	...
	if (create_polyhedron_from_top(&polyhedron1, man, top, dim, lincons0)) {
		...
		long assignment_values2[3] = { -198, -1, -1 };
		...
		opt_pk_array_t* assign_result = assign(dim, man, polyhedron1,
				assignment_values1, 0);
		opt_pk_internal_t * assign_internal = opt_pk_init_from_manager(man,
				ELINA_FUNID_ASSIGN_LINEXPR_ARRAY);

		opt_pk_array_t* project_result = project(dim, man, polyhedron1, 0);

		if (assign_internal->exn != ELINA_EXC_OVERFLOW) {
			printf("assign result is top: %d \n", opt_pk_is_top(man, assign_result));
			printf("project result is top: %d \n", opt_pk_is_top(man, project_result));
			printf("assign(polyhedron1) <= project(polyhedron1): ");
			printf("%d\n", opt_pk_is_leq(man, assign_result, project_result));
		}
