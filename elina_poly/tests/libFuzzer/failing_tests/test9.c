...
int main(int argc, char **argv) {
	...
	if (create_polyhedron_from_bottom(&polyhedron1, man, top, bottom, dim,
		...
		long assignment_values1[3] = { 5, 1024, 0 };
		...
		if (create_polyhedron_from_bottom(&polyhedron2, man, top, bottom, dim,
			...
			elina_lincons0_array_t p1 = opt_pk_to_lincons_array(man,
					polyhedron1);
			fprintf(stdout, "P1: ");
			elina_lincons0_array_print(&p1, NULL);
			fflush(stdout);
			elina_lincons0_array_t p2 = opt_pk_to_lincons_array(man,
					polyhedron2);
			fprintf(stdout, "P2: ");
			elina_lincons0_array_print(&p2, NULL);
			fflush(stdout);

			opt_pk_array_t* join12 = opt_pk_join(man, false, polyhedron1,
					polyhedron2);
			elina_lincons0_array_t a3 = opt_pk_to_lincons_array(man, join12);
			fprintf(stdout, "Join 12: ");
			elina_lincons0_array_print(&a3, NULL);
			fflush(stdout);
			opt_pk_internal_t * join12_internal = opt_pk_init_from_manager(man,
					ELINA_FUNID_JOIN);

			elina_lincons0_array_t p3 = opt_pk_to_lincons_array(man,
					polyhedron1);
			fprintf(stdout, "P3: ");
			elina_lincons0_array_print(&p3, NULL);
			fflush(stdout);
			elina_lincons0_array_t p4 = opt_pk_to_lincons_array(man,
					polyhedron2);
			fprintf(stdout, "P4: ");
			elina_lincons0_array_print(&p4, NULL);
			fflush(stdout);

			opt_pk_array_t* join21 = opt_pk_join(man, false, polyhedron2,
					polyhedron1);
			elina_lincons0_array_t a4 = opt_pk_to_lincons_array(man, join21);
			fprintf(stdout, "Join 21: ");
			elina_lincons0_array_print(&a4, NULL);
			fflush(stdout);
			opt_pk_internal_t * join21_internal = opt_pk_init_from_manager(man,
					ELINA_FUNID_JOIN);
			...
			if (join12_internal->exn != ELINA_EXC_OVERFLOW
				...
				//join is commutative
				printf(
						"polyhedron1 join polyhedron2 equals polyhedron2 join polyhedron1: ");
				printf("%d\n", opt_pk_is_eq(man, join12, join21));
				printf(
						"polyhedron1 join polyhedron2 less equals polyhedron2 join polyhedron1: ");
				printf("%d\n", opt_pk_is_leq(man, join12, join21));
				printf(
						"polyhedron2 join polyhedron1 less equals polyhedron1 join polyhedron2: ");
				printf("%d\n", opt_pk_is_eq(man, join21, join12));
			}
		}
	}
	return 0;
