...
bool opt_poly_meet_matrix(bool meet, bool lazy, elina_manager_t* man,
		opt_pk_t* op, opt_pk_t* oa, opt_matrix_t* oc) {
	...
	if (lazy) {
		...
		if (op != oa) {
			...
			opt_satmat_resize_cols(oa->satC, opt_bitindex_size(oa->C->nbrows));
		}
...
bool opt_poly_meet_elina_lincons_array(bool lazy, elina_manager_t* man,
		opt_pk_t* op, opt_pk_t* oa, elina_lincons0_array_t* array) {
	opt_matrix_t* mat;
	bool quasilinear;
	opt_pk_internal_t* opk = (opt_pk_internal_t*) man->internal;
	...
	bool exact = opt_matrix_set_elina_lincons0_array(opk, &mat, array,
			op->intdim, op->realdim, true);
...
comp_list_t * linexpr0_to_comp_list(opt_pk_internal_t * opk,
...
comp_list_t * lincons0_to_comp_list(opt_pk_internal_t * opk,
		elina_lincons0_t * cons) {
	elina_linexpr0_t * expr = cons->linexpr0;
	return linexpr0_to_comp_list(opk, expr);
}

array_comp_list_t * lincons0_array_to_array_comp_list(opt_pk_internal_t *opk,
		elina_lincons0_array_t* array, unsigned short int n, char * is_trivial) {
...
elina_linexpr0_t * copy_linexpr0_with_comp_list(opt_pk_internal_t *opk,
		elina_linexpr0_t * src, unsigned short int * ca,
...
void copy_lincons0_with_comp_list(opt_pk_internal_t *opk,
		elina_lincons0_t * dst, elina_lincons0_t * src, unsigned short int * ca,
		unsigned short int comp_size) {
...
opt_pk_array_t* opt_pk_meet_lincons_array_cons(elina_manager_t* man,
		bool destructive, opt_pk_array_t* oa, elina_lincons0_array_t* array) {
	//printf(".");
	...
	//fflush(stdout);
	opt_pk_internal_t* opk = opt_pk_init_from_manager(man,
	...
	array_comp_list_t * aclb = lincons0_array_to_array_comp_list(opk, array,
			oa->maxcols, is_trivial);
	...
	unsigned short int ** ca_arr = (unsigned short int **) malloc(
			num_comp * sizeof(unsigned short int *));
	unsigned short int * comp_size_map = (unsigned short int *) calloc(num_comp,
	 ...
	 ***********************************/
	elina_lincons0_array_t * arr = (elina_lincons0_array_t *) malloc(
			num_comp * sizeof(elina_lincons0_array_t));
	unsigned short int * rmapb = (unsigned short int *) calloc(size,
			sizeof(unsigned short int));
	...
	unsigned short int * rmapa = (unsigned short int *) calloc(num_compa,
			sizeof(unsigned short int));
	...
	for (k = 0; k < num_comp; k++) {
		...
		if (!nbmapb[k]) {
			...
			if (destructive) {
				...
				poly[k]->nbeq = poly_a[l]->nbeq;
				poly[k]->F =
						poly_a[l]->F ? opt_matrix_copy(poly_a[l]->F) : NULL;
				poly[k]->satF =
						poly_a[l]->satF ?
								opt_satmat_copy(poly_a[l]->satF) : NULL;
				poly[k]->satC =
						poly_a[l]->satC ?
								opt_satmat_copy(poly_a[l]->satC) : NULL;
				poly[k]->nbline = poly_a[l]->nbline;
			}
		} else {
			...
			poly[k]->F = opt_matrix_alloc(
					nbgenmapa[k] + 2 * num_vertex[k] + nblines + 1,
					comp_size + 2, false);
			num_vertex[k] = 0;
			nblines = 0;
			for (k1 = 0; k1 < comp_size; k1++) {
	...
	meet_cons_with_map(opk, oa, poly, rmapa, ca_arr, counterC, pos_con_map,
			disjoint_map);
	/*************************
	...
	cartesian_product_vertices_with_map(oa, poly, rmapa, ca_arr, num_vertex_a,
			num_vertex, counterF, disjoint_map);

	// meet of rays
	meet_rays_with_map(oa, poly, rmapa, ca_arr, num_vertex_a, counterF,
			disjoint_map);
	char * exc_map = (char *) calloc(num_comp, sizeof(char));
	is_bottom = false;
	for (k = 0; k < num_comp; k++) {
		if (nbmapb[k] && !is_bottom) {
			poly[k]->satC = opt_satmat_alloc(poly[k]->F->nbrows,
					opt_bitindex_size(poly[k]->C->nbrows));
			is_bottom = opt_poly_meet_elina_lincons_array(
					opk->funopt->algorithm < 0, man, poly[k], poly[k], arr + k);
			if (opk->exn) {
				if (opk->exn != ELINA_EXC_OVERFLOW) {
					opk->exn = ELINA_EXC_NONE;
				}
...
opt_pk_array_t* opt_pk_meet_lincons_array(elina_manager_t* man,
	...
	start_timing();
#endif
	opt_pk_array_t * op = opt_pk_meet_lincons_array_cons(man, destructive, oa,
			array);
#if defined(TIMING)
 ...
 *************************************************/
opt_pk_array_t* opt_pk_meet_tcons_array(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, elina_tcons0_array_t* array) {
	opt_pk_array_t *op;
	op = elina_generic_meet_intlinearize_tcons_array(man, destructive, oa,
			array, ELINA_SCALAR_MPQ, ELINA_LINEXPR_LINEAR,
...
bool are_vertex_disjoint(opt_pk_internal_t *opk, opt_numint_t * v1,
		opt_numint_t * v2, unsigned short int size) {
	unsigned short int j;
...
bool is_vertex_included(opt_pk_internal_t *opk, opt_numint_t * v1,
		opt_numint_t * v2, unsigned short int size) {
	unsigned short int j;
	char * map1 = (char *) calloc(size - 2, sizeof(char));
	char * map2 = (char *) calloc(size - 2, sizeof(char));
...
void combine_vertex(opt_pk_internal_t *opk, opt_numint_t *v1, opt_numint_t *v2,
 ...
 *******************************/
opt_pk_array_t * opt_poly_join_gen(elina_manager_t *man, opt_pk_array_t *oa,
		opt_pk_array_t *ob, bool destructive) {
	...
	opt_pk_array_t *op =
			destructive ? oa : opt_pk_array_alloc(NULL, NULL, maxcols);
	if (oa->is_bottom || !acla) {
		if (destructive) {
	 ...
	 ***************************/
	unsigned short int * rmapb = (unsigned short int *) calloc(num_compb,
			sizeof(unsigned short int));
	...
	for (k = 0; k < num_comp; k++) {
		...
		poly1[k]->C = opt_matrix_alloc(nbmapCa[k] + 1 + nbeq, comp_size + 2,
				false);
	...
	for (k = 0; k < num_comp; k++) {
		...
		if (pos_con_map[k] == 1) {
			...
			poly[k2]->satF = opt_satmat_alloc(poly[k2]->C->nbrows,
					opt_bitindex_size(poly[k2]->F->nbrows));
			combine_satmat(opk, poly[k2], comp_size, poly[k2]->F->nbrows,
					false);
	...
	if (flag) {
		...
		count = cartesian_product_vertices_one_comp(poly1, acl, ca_arr,
				num_vertex1, poly[0], count, ca, pos_con_map);
		//combine all overlapping rays of A into one		

		meet_rays_one_comp(poly1, acl, ca_arr, nblinemapa, poly[0], num_vertex1,
				ca, count, pos_con_map);
		...
		count = cartesian_product_vertices_one_comp(poly2, acl, ca_arr,
				num_vertex2, poly[0], poly[0]->F->nbrows, ca, pos_con_map);
		//combine all overlapping rays of B into one

		meet_rays_one_comp(poly2, acl, ca_arr, NULL, poly[0], num_vertex2, ca,
				count, pos_con_map);
		free(ca);
		...
		/************************
		 Combine satmat of A
		 *************************/
		poly[0]->satF = opt_satmat_alloc(C->nbrows,
				opt_bitindex_size(F->nbrows));
...
opt_pk_array_t* opt_pk_join(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, opt_pk_array_t* ob) {
	opt_pk_array_t * res;
#if defined (TIMING)
	start_timing();
#endif
...
void opt_poly_meet(bool meet, bool lazy, elina_manager_t* man,
	...
	for (k = 0; k < num_comp; k++) {
		unsigned short int comp_size = cl->size;
		poly[k] = opt_poly_alloc(comp_size, 0);
		poly[k]->C = opt_matrix_alloc(nbmapa[k] + nbmapb[k] + 1, comp_size + 2,
				false);
		poly[k]->C->p[0][0] = 1;
		poly[k]->C->p[0][1] = 1;
...
opt_pk_array_t* opt_pk_meet_cons(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, opt_pk_array_t* ob) {
	opt_pk_internal_t* opk = opt_pk_init_from_manager(man, ELINA_FUNID_MEET);
	opt_pk_array_t* op =
			destructive ? oa : opt_pk_array_alloc(NULL, NULL, oa->maxcols);
	opt_pk_t ** poly_a = oa->poly;
	array_comp_list_t * acla = oa->acl;
	array_comp_list_t * aclb = ob->acl;
	...
	opt_poly_meet(true, opk->funopt->algorithm < 0, man, op, oa, ob);

	return op;
}

opt_pk_array_t* opt_pk_meet(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, opt_pk_array_t* ob) {
#if defined (TIMING)
	start_timing();
#endif
