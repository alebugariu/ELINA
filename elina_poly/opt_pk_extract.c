...
elina_interval_t* opt_pk_bound_dimension_gen(elina_manager_t* man,
		opt_pk_array_t* oa, elina_dim_t dim) {
...
elina_interval_t* opt_pk_bound_linexpr(elina_manager_t* man, opt_pk_array_t* oa,
...
elina_lincons0_array_t opt_pk_to_lincons_array(elina_manager_t* man,
	...
	for (k = 0; k < num_compa; k++) {
		...
		for (i = 0; i < oc->nbrows; i++) {
			if (!opt_vector_is_dummy_constraint(opk, oc->p[i], oc->nbcolumns)) {
				array.p[l] = opt_lincons0_of_vector(opk, oc->p[i], ca,
						cla->size, oa->maxcols);
				l++;
			}
		}
...
elina_interval_t** opt_pk_to_box(elina_manager_t* man, opt_pk_array_t* op) {
#if defined(TIMING)
	start_timing();
#endif
	elina_interval_t** interval;
