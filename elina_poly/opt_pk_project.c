...
int select_variable_gauss(opt_pk_internal_t *opk, opt_matrix_t *oc,
		opt_numint_t *cons, size_t nbeq, elina_dim_t *tdim, size_t size,
		char *rmap) {
	int res = -1;
...
bool gauss_project(opt_pk_internal_t *opk, opt_pk_t *o, elina_dim_t *tdim,
		size_t *tsize, size_t * proj) {
	size_t nbeq = o->nbeq;
	opt_matrix_t * oc = o->C;
		...
		if (s == -1) {
			...
			while (l < nbrows) {
				...
				if (pl[s] == 0) {
					...
					if (pl[s] > 0) {
						opt_vector_combine(opk, pl, npi, pl, s, nbcolumns,
								true);
					} else {
...
void select_variable(opt_pk_internal_t * opk, elina_dim_t *tdim, size_t size,
		opt_matrix_t * oc, int *nbadd, int *maxadd) {
...
void opt_poly_projectforget_array(bool project, elina_manager_t *man,
		opt_pk_t* op, opt_pk_t* oa, elina_dim_t* tdim, size_t size,
		bool destructive) {
...
opt_pk_array_t* opt_pk_forget_array(elina_manager_t* man, bool destructive,
		opt_pk_array_t* oa, elina_dim_t* tdim, size_t size, bool project) {
	opt_pk_internal_t* opk = opt_pk_init_from_manager(man,
			ELINA_FUNID_FORGET_ARRAY);
		...
		while (k < num_compa) {
			...
			if (!flag) {
				...
				if (project) {
					...
					/* Forget */
					opt_matrix_t *F = opt_matrix_alloc(size, src->F->nbcolumns,
							false);
				...
				if (opk->funopt->flag_best_wanted
					...
					man->result.flag_best = man->result.flag_exact = real;
				} else {
					man->result.flag_best = man->result.flag_exact = src->intdim
							== 0;
				}
				...
				dst_mat = opt_matrix_alloc(nbcons, ncomp_size + opk->dec,
						src_mat->_sorted);
			...
			if (cl->size) {
				...
				if (dim_size) {
					...
					if (project) {
						...
						/* Forget */
						opt_matrix_t *F = opt_matrix_alloc(size,
								src->F->nbcolumns, false);
						for (i = 0; i < dim_size; i++) {
							F->p[i][opk->dec + tdimk[i]] = 1;
						}
						opt_matrix_sort_rows(opk, F);
					...
					free(nca);
					poly[k1]->F = dst_mat;
					opt_poly_chernikova(man, poly[k1],
