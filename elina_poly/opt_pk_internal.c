...
opt_pk_internal_t* opt_pk_internal_alloc(bool strict) {
	opt_pk_internal_t* opk = (opt_pk_internal_t*) malloc(
			sizeof(opt_pk_internal_t));
...
void opt_pk_internal_clear(opt_pk_internal_t* opk) {
	size_t i;
	if (opk->vector_numintp)
		opt_vector_free(opk->vector_numintp, opk->maxcols);
	opk->vector_numintp = NULL;
	if (opk->vector_tmp)
		opt_vector_free(opk->vector_tmp, 5);
	opk->vector_tmp = NULL;
	if (opk->matrix_dimp)
		free(opk->matrix_dimp);
	opk->matrix_dimp = NULL;
	opk->matrix_acc = 0;
	opk->matrix_prod = 0;

	if (opk->cherni_intp)
		free(opk->cherni_intp);
	...
	if (opk->poly_numintp)
		opt_vector_free(opk->poly_numintp, opk->maxcols);
	opk->poly_numintp = NULL;

	if (opk->poly_numintp2)
		opt_vector_free(opk->poly_numintp2, opk->maxcols);
	...
	if (opk->poly_dimp2)
		free(opk->poly_dimp2);
	opk->poly_dimp2 = NULL;
	if (opk->poly_fold_dimp)
		free(opk->poly_fold_dimp);
...
elina_manager_t* opt_pk_manager_alloc(bool strict) {
	...
	man = elina_manager_alloc(
			...
			"1.0 with Long Long Int", opk,
			(void (*)(void*)) opt_pk_internal_free);
	funptr = man->funptr;
	...
	funptr[ELINA_FUNID_IS_DIMENSION_UNCONSTRAINED] =
			&opt_pk_is_dimension_unconstrained;
	...
	funptr[ELINA_FUNID_SUBSTITUTE_LINEXPR_ARRAY] =
			&opt_pk_substitute_linexpr_array;
