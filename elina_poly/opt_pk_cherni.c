...
void opt_cherni_resize(opt_matrix_t* mat, opt_satmat_t* osc) {
	...
	size_t currentsize =
			mat->_maxrows >= osc->_maxrows ? mat->_maxrows : osc->_maxrows;
...
size_t opt_cherni_conversion(opt_pk_internal_t* opk, opt_matrix_t* con,
		size_t begin, opt_matrix_t* ray, opt_satmat_t* osc, size_t nbline) {
...
//    unsigned short int * nz = build_index_vector(con->p[k.index],nbcols); 
		for (i = 0; i < nbrows; i++) {
			...
			//     con->p[k.index],nz);
			ray->p[i][0] = opt_vector_product(opk, ray->p[i], con->p[k.index],
					nbcols);
			if (opk->exn) {
			...
			if (is_inequality && sup_bound == nbrows) {
				...
				if (sup_bound == nbline) { /* no ray satisfies the constraint */
					...
					for (i = equal_bound; i < sup_bound; i++) {
						for (j = sup_bound; j < bound; j++) {
							...
							if (nbcommonconstraints + nbline >= nbcols - 3) { /* possibly adjacent */
								...
								if (!redundant) { /* if not */
									if (opk->funopt->max_object_size
											&& nbrows * (nbcols - opk->dec)
													> opk->funopt->max_object_size) {
										/* out of space overflow */
										opk->exn = ELINA_EXC_OUT_OF_SPACE;
										goto cherni_conversion_exit0;
									}
									if (nbrows >= opt_matrix_get_maxrows(ray)
											|| nbrows >= osc->_maxrows) {
									...
									//start_timing();
									opt_matrix_combine_rows(opk, ray, j, i,
											nbrows, 0, flag);
					...
					/* with those that don't verify the constraint */
					{
						...
						if (is_inequality) {
							j = sup_bound;
							for (l = equal_bound; l < sup_bound; l++)
								opt_satmat_set(osc, l, k);
						} else {
							j = equal_bound;
						}
						i = nbrows;
	...
	cherni_conversion_exit0: opt_bitstring_free(bitstringp);
...
int opt_cherni_simplify(opt_pk_internal_t* opk, opt_matrix_t* con,
		opt_matrix_t* ray, opt_satmat_t* satf, size_t nbline) {
	size_t i, j;
	long int nb, nbj;
	size_t nbeq, rank;
	size_t w;
	...
	for (nbeq = 0; nbeq < nbcons; nbeq++) {
		if (opt_numint_sgn(con->p[nbeq][0]))
			break;
	}
...
void opt_cherni_minimize(opt_pk_internal_t* opk, bool con_to_ray, opt_pk_t* op) {
	...
	C = op->C;
	assert(
			opt_matrix_is_sorted(C) && op->F==NULL && op->satC==NULL && op->satF==NULL);
	//printf("cherni start %d %d\n",C->nbrows, C->nbcolumns);
	//opt_matrix_fprint(stdout,C);
	//fflush(stdout);
	/* initialization of F and sat */
	F = opt_matrix_alloc(
	OPT_CHERNI_FACTOR * opt_uint_max(C->nbrows, C->nbcolumns - 1), C->nbcolumns,
			false);
...
void opt_cherni_add_and_minimize(opt_pk_internal_t* opk, bool con_to_ray,
		opt_pk_t* op, size_t start) {
	size_t i;
	bool special;
	opt_matrix_t* C;
	opt_matrix_t* F;
	...
	op->nbline = opt_cherni_conversion(opk, C, start, F, satC, op->nbline);
