		...
		if (op_k->F) {
			...
			if (opk->funopt->algorithm >= 0) {
				...
				man->result.flag_exact = man->result.flag_best =
						op_k->intdim > 0 && op_k->F ? false : true;
				if (!op_k->F) {
#if defined(TIMING)
					record_timing(is_bottom_time);
#endif
...
bool opt_pk_is_top(elina_manager_t* man, opt_pk_array_t* o) {
	...
	opt_numint_t * comps = (opt_numint_t *) malloc(
			num_comp * sizeof(opt_numint_t));
	for (k = 0; k < num_comp; k++) {
...
unsigned short int * insertion_sort(unsigned short int *arr,
		unsigned short int size) {
	unsigned short int * res = (unsigned short int *) malloc(
...
opt_numint_t* tab, bool is_strict) {
	...
	if (opt_numint_sgn(tab[0]) == 0) {
		...
		for (i = 0; i < F->nbrows; i++) {
			...
			opt_numint_t prod = opt_vector_product_strict(opk, F->p[i], tab,
					F->nbcolumns);
		...
		for (i = 0; i < F->nbrows; i++) {
			...
			if (sign < 0) {
				...
				if (opt_numint_sgn(F->p[i][0]) == 0) {
					...
					if (is_strict && sign == 0
							&& (opk->strict ?
									opt_numint_sgn(F->p[i][opt_polka_eps]) > 0 :
									true)) {
						//free(fpi);
						return false;
					}
...
bool opt_poly_leq(opt_pk_internal_t * opk, opt_matrix_t * C, opt_matrix_t * F) {
	...
	size_t i;
	for (i = 0; i < nbrows; i++) {
		if (!opt_generators_sat_vector(opk, F, C->p[i],
				opk->strict && opt_numint_sgn(C->p[i][opt_polka_eps]) < 0)) {
			return false;
		}
	}
	return true;
...
bool opt_pk_is_leq_gen(elina_manager_t * man, opt_pk_array_t *oa,
		opt_pk_array_t *ob) {
	...
	unsigned short int * comp_size_arr = (unsigned short int *) malloc(
			num_comp * sizeof(unsigned short int));
	...
	unsigned short int ** ind_melina_a = (unsigned short int **) malloc(
			num_compa * sizeof(unsigned short int *));
	...
	unsigned short int * rmapb = (unsigned short int *) calloc(num_compb,
			sizeof(unsigned short int));
	unsigned short int ** ind_melina_b = (unsigned short int **) malloc(
			num_compb * sizeof(unsigned short int *));
	...
	for (k = 0; k < num_comp; k++) {
		...
		tmp[k]->F = opt_matrix_alloc(nbgena[k] + 2 * num_vertex1[k] + nblines,
				comp_size + 2, false);
		num_vertex1[k] = 0;
	 ...
	 *******************/
	cartesian_product_vertices(oa, tmp, rmapa, ca_arr, num_vertex_a,
			num_vertex1, counter);
	...
	for (kb = 0; kb < num_compb; kb++) {
		...
		for (i = 0; i < nbconsb; i++) {
			...
			opt_numint_t * cpi = opt_map_vector(obk->C->p[i], ind_melina_b[kb],
					comp_size_arr[indb], obk->C->nbcolumns);

			sat = opt_generators_sat_vector(opk, tmp[indb]->F, cpi,
					opk->strict
...
/* ====================================================================== */
void fuse_generators_intersecting_blocks(opt_matrix_t *F, opt_pk_t ** poly_a,
		array_comp_list_t *acla, unsigned short int *ca, size_t * num_vertex_a,
		char *intersect_map, unsigned short int maxcols) {
	unsigned short int num_compa = acla->size;
	...
	size_t counter = 0;
	comp_list_t * cla = acla->head;
	unsigned short int ** ca_a_arr = (unsigned short int **) malloc(
			num_compa * sizeof(unsigned short int *));
	for (k = 0; k < num_compa; k++) {
		if (!intersect_map[k]) {
			cla = cla->next;
...
bool opt_pk_sat_lincons(elina_manager_t* man, opt_pk_array_t* oa,
	...
	elina_lincons0_t * new_lincons0 = (elina_lincons0_t *) malloc(
			sizeof(elina_lincons0_t));
	new_lincons0->scalar = NULL;
	for (k = 0; k < num_comp; k++) {
		if (!is_disjoint(cl, clb, maxcols)) {
			comp_size = cl->size;
			ca = to_sorted_array(cl, maxcols);
			copy_lincons0_with_comp_list(opk, new_lincons0, lincons0, ca,
					comp_size);
			break;
		}
		cl = cl->next;
	}
	opt_matrix_t * F = opt_matrix_alloc(nbgen + num_vertex, comp_size + 2,
			false);
	fuse_generators_intersecting_blocks(F, poly_a, acla, ca, num_vertex_a,
			intersect_map, maxcols);
	free(ca);
	free(map);
	free(intersect_map);
	...
	if (!elina_linexpr0_is_quasilinear(lincons0->linexpr0)) {
		...
		exact = quasilinearize_elina_lincons0(new_lincons0, env, false,
				ELINA_SCALAR_MPQ) && exact;
		elina_interval_array_free(env, dim);
	}

	sat = opt_vector_set_elina_lincons0_sat(opk, opk->poly_numintp,
			new_lincons0, comp_size, 0, true);
	if (sat) {
		if (F->nbrows == 0) {
			sat = false;
		} else {
			sat = opt_generators_sat_vector(opk, F, opk->poly_numintp,
	...
	man->result.flag_exact =
			man->result.flag_best =
					sat ? true : (
							((opk->funopt->flag_exact_wanted
									|| opk->funopt->flag_best_wanted) && exact
									&& elina_linexpr0_is_real(
											lincons0->linexpr0, dim)) ?
									true : false);
...
bool opt_pk_sat_tcons(elina_manager_t* man, opt_pk_array_t* oa,
		elina_tcons0_t* cons) {
	size_t dim;
	opt_pk_internal_t* opk = opt_pk_init_from_manager(man,
			ELINA_FUNID_SAT_LINCONS);
	 ...
	 ********/
	elina_lincons0_t *lincons0 = (elina_lincons0_t *) malloc(
			sizeof(elina_lincons0_t));
	...
	elina_intlinearize_elina_tcons0(lincons0, cons, env, dim, ELINA_SCALAR_MPQ);
	...
	free(ca);
	free(intersect_map);
	bool sat = opt_vector_set_elina_lincons0_sat(opk, opk->poly_numintp,
			new_lincons0, comp_size, 0, true);
