...
opt_oct_t* opt_oct_forget_array(elina_manager_t* man, bool destructive,
	...
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man,
			ELINA_FUNID_FORGET_ARRAY, 0);
	...
	if (!o->closed && !o->m)
		...
		if (o->closed) {
			...
			if (num_incomplete || o->intdim)
				flag_incomplete;
			if (project)
				return opt_oct_set_mat(pr, o, oo, NULL, destructive);
			else
				return opt_oct_set_mat(pr, o, NULL, oo, destructive);
		} else {
...
opt_oct_t* opt_oct_add_dimensions(elina_manager_t* man, bool destructive,
	...
	if (!src)
		...
		opt_hmat_addrem_dimensions(dst, src, dimchange->dim, nb, 1, o->dim,
				true);
		if (dst->is_dense) {
...
opt_oct_t* opt_oct_remove_dimensions(elina_manager_t* man, bool destructive,
	...
	/*printf("Remove IN\n");
	 ...
	 fflush(stdout);*/
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man,
			ELINA_FUNID_REMOVE_DIMENSIONS, 0);
...
opt_oct_t* opt_oct_expand(elina_manager_t* man, bool destructive, opt_oct_t* o,
		elina_dim_t dim, size_t n) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_EXPAND,
			0);
	opt_oct_mat_t* src = o->closed ? o->closed : o->m;
	size_t i, j;
	if (n == 0) {
		...
		if (!src->is_dense) {
			for (i = 0; i < n; i++) {
				...
				if (cj != NULL) {
					...
					while (c != NULL) {
						...
						mm[opt_matpos2(2 * (pos + i), 2 * j)] = mm[opt_matpos2(
								2 * dim, 2 * j)];
						mm[opt_matpos2(2 * (pos + i) + 1, 2 * j)] =
								mm[opt_matpos2(2 * dim + 1, 2 * j)];
						mm[opt_matpos2(2 * (pos + i), 2 * j + 1)] =
								mm[opt_matpos2(2 * dim, 2 * j + 1)];
						mm[opt_matpos2(2 * (pos + i) + 1, 2 * j + 1)] =
								mm[opt_matpos2(2 * dim + 1, 2 * j + 1)];
						insert_comp(cl, j);
						c = c->next;
					}
				...
				if (cp != NULL) {
					if (cp == cj) {
						mm[opt_matpos2(2 * (pos + i), 2 * (pos + i))] =
								mm[opt_matpos2(2 * dim, 2 * (pos + i))];
						mm[opt_matpos2(2 * (pos + i) + 1, 2 * (pos + i) + 1)] =
								mm[opt_matpos2(2 * dim + 1, 2 * (pos + i) + 1)];
				...
				if (cj != NULL) {
					mm[opt_matpos2(2 * (pos + i), 2 * (pos + i) + 1)] =
							mm[opt_matpos2(2 * dim, 2 * dim + 1)];
					//flag = 1;
					//}
					//if(mm[opt_matpos2(2*dim+1,2*dim)] != INFINITY){
					mm[opt_matpos2(2 * (pos + i) + 1, 2 * (pos + i))] =
							mm[opt_matpos2(2 * dim + 1, 2 * dim)];
					//flag = 1;
				} else {
					mm[opt_matpos2(2 * (pos + i), 2 * (pos + i) + 1)] =
							INFINITY;
			...
			for (i = 0; i < n; i++) {
				...
				for (j = 0; j < ((2 * dim) / v_length) * v_length; j++) {
					mm[opt_matpos(2 * (pos + i), j)] =
							mm[opt_matpos(2 * dim, j)];
					mm[opt_matpos(2 * (pos + i) + 1, j)] = mm[opt_matpos(
							2 * dim + 1, j)];
					//count = count + 2;
				}
#endif
				...
				for (j = 2 * dim + 2; j < 2 * (o->dim + n); j++) {
					mm[opt_matpos2(2 * (pos + i), j)] = mm[opt_matpos(j ^ 1,
							2 * dim + 1)];
					mm[opt_matpos2(2 * (pos + i) + 1, j)] = mm[opt_matpos(j ^ 1,
							2 * dim)];
					//count = count + 2;
				}
...
opt_oct_t* opt_oct_fold(elina_manager_t* man, bool destructive, opt_oct_t* o,
		elina_dim_t* tdim, size_t size) {

	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_FOLD,
			0);
	opt_oct_mat_t* src;
	opt_oct_mat_t* dst;
	opt_oct_t* r;
	...
	if (!src)
		...
		if (!src->is_dense) {
			...
			for (j = 0; j < tdim[0]; j++) {
				...
				for (i = 1; i < size; i++) {
					...
					if (cj != cti) {
						...
						*mm2 = max(*mm2,
								m[opt_matpos2(tdim[i] * 2 + 1, 2 * j)]);
						*mm3 = max(*mm3,
								m[opt_matpos2(tdim[i] * 2, 2 * j + 1)]);
						*mm4 = max(*mm4,
								m[opt_matpos2(tdim[i] * 2 + 1, 2 * j + 1)]);
					}
				}
			}
			...
			{
				double* mm1 = oo->mat
						+ opt_matpos2(tdim[0] * 2, tdim[0] * 2 + 1);
				double* mm2 = oo->mat
						+ opt_matpos2(tdim[0] * 2 + 1, tdim[0] * 2);
				for (i = 1; i < size; i++) {
					comp_list_t *cti = find(oo->acl, tdim[i]);
					if (cti == NULL) {
						*mm1 = INFINITY;
						...
						*mm1 = max(*mm1,
								m[opt_matpos2(tdim[i] * 2, tdim[i] * 2 + 1)]);
						*mm2 = max(*mm2,
								m[opt_matpos2(tdim[i] * 2 + 1, tdim[i] * 2)]);
					}
				}
			}
		...
		opt_hmat_addrem_dimensions(dst, oo, tdim + 1, size - 1, 1, o->dim,
				false);
		if (dst->is_dense) {
			free_array_comp_list(dst->acl);
		}
