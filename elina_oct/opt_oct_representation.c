...
int opt_oct_size(elina_manager_t* man, opt_oct_t* o) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_ASIZE,
			0);
	if (!o->m)
		return 1;
...
opt_oct_t* opt_oct_copy(elina_manager_t* man, opt_oct_t* o) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_COPY,
			0);
...
void opt_oct_free(elina_manager_t* man, opt_oct_t* o) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_FREE,
			0);
...
opt_oct_t* opt_oct_bottom(elina_manager_t* man, int intdim, int realdim) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_BOTTOM,
			0);
...
opt_oct_t* opt_oct_of_box(elina_manager_t* man, size_t intdim, size_t realdim,
		elina_interval_t ** t) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_OF_BOX,
			0);
	opt_oct_t* r = opt_oct_alloc_internal(pr, intdim + realdim, intdim);
	...
	if (!t)
		return r; /* empty */
	for (i = 0; i < r->dim; i++)
		if (elina_scalar_cmp(t[i]->inf, t[i]->sup) > 0)
			return r; /* empty */
	r->closed = opt_hmat_alloc_top(r->dim);
	for (i = 0; i < r->dim; i++) {
		if (opt_oct_of_interval(pr, r->closed, i, t[i], true)) {
	...
	unsigned short int *index = (unsigned short int *) calloc(
			2 * (2 * r->dim + 1), sizeof(unsigned short int));
...
int opt_oct_hash(elina_manager_t* man, opt_oct_t* o) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_HASH,
			0);
...
opt_oct_t* opt_oct_closure(elina_manager_t *man, bool destructive, opt_oct_t *o) {
	opt_oct_internal_t *pr = (opt_oct_internal_t *) opt_oct_init_from_manager(
			man, ELINA_FUNID_CLOSURE, 0);
	if (destructive)
		return o;
...
void opt_oct_fprint(FILE* stream, elina_manager_t* man, opt_oct_t * a,
		char** name_of_dim) {
#if defined(TIMING)
...
elina_manager_t* opt_oct_manager_alloc(void) {
	...
	man->funptr[ELINA_FUNID_IS_DIMENSION_UNCONSTRAINED] =
			&opt_oct_is_dimension_unconstrained;
	...
	man->funptr[ELINA_FUNID_ASSIGN_LINEXPR_ARRAY] =
			&opt_oct_assign_linexpr_array;
