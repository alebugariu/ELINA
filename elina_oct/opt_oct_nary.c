...
opt_oct_t* opt_oct_meet(elina_manager_t* man, bool destructive, opt_oct_t* o1,
	...
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_MEET,
			0);
	opt_oct_mat_t* oo;
...
opt_oct_t* opt_oct_join(elina_manager_t* man, bool destructive, opt_oct_t* o1,
	...
	/*printf("JOIN IN\n");
	 ...
	 fflush(stdout);*/
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man, ELINA_FUNID_JOIN,
			0);
				...
				if (elina_scalar_infty(inf)) {
					...
					if (b > 0) {
						precision = precision + 0.5
								- (0.5 * log(b + 1)) / (1 + log(b + 1));
					} else if (b < 0) {
						precision = precision + 0.5
								+ (0.5 * log(1 - b)) / (1 + log(1 - b));
					} else {
						precision = precision + 0.5;
					}
					...
					if (a > 0) {
						precision = precision + 0.5
								+ (0.5 * log(a + 1)) / (1 + log(a + 1));
					} else if (a < 0) {
						precision = precision + 0.5
								- (0.5 * log(1 - a)) / (1 + log(1 - a));
					} else {
						precision = precision + 0.5;
					}
				} else {
...
opt_oct_t* opt_oct_join_array(elina_manager_t* man, opt_oct_t** tab,
	...
	for (k = 0; k < size; k++) {
		...
		if (algo >= 0)
			opt_oct_cache_closure(pr, tab[k]);
		/* skip definitely empty */
		if (!tab[k]->m && !tab[k]->closed)
			continue;
		if (!oo)
			/* first non-empty */
			oo = opt_hmat_copy(tab[k]->closed ? tab[k]->closed : tab[k]->m,
					r->dim);
		else {
			/* not first non-empty */
...
elina_abstract0_opt_oct_widening_thresholds(elina_manager_t* man,
		elina_abstract0_t* a1, elina_abstract0_t* a2, elina_scalar_t** array,
		size_t nb) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man,
			ELINA_FUNID_WIDENING, 0);
	...
	if ((man->library != a1->man->library)
		...
		return abstract0_of_opt_oct(man,
				opt_oct_alloc_top(pr, o->dim, o->intdim));
	}
	return abstract0_of_opt_oct(man,
			opt_oct_widening_thresholds(man, a1->value, a2->value, array, nb));
}
...
elina_abstract0_t* elina_abstract0_opt_oct_narrowing(elina_manager_t* man,
	...
	return abstract0_of_opt_oct(man,
			opt_oct_narrowing(man, a1->value, a2->value));
}
...
elina_abstract0_t*
elina_abstract0_opt_oct_add_epsilon(elina_manager_t* man, elina_abstract0_t* a1,
		elina_scalar_t* epsilon) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man,
			ELINA_FUNID_WIDENING, 0);
	opt_oct_t* o = (opt_oct_t*) (a1->value);
			...
			while (cl != NULL) {
				...
				if (!oo2->ti) {
					for (int i = 0; i < comp_size; i++) {
						...
						for (int j = 0; j <= i; j++) {
							int j1 = ca[j];
							handle_binary_relation(m2, oo2->acl, i1, j1,
									o1->dim);
						}
					}
				}
...
elina_abstract0_opt_oct_add_epsilon_bin(elina_manager_t* man,
		elina_abstract0_t* a1, elina_abstract0_t* a2, elina_scalar_t* epsilon) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man,
			ELINA_FUNID_WIDENING, 0);
	...
	if ((man->library != a1->man->library)
			|| (man->library != a2->man->library)) {
		return abstract0_of_opt_oct(man,
				opt_oct_alloc_top(pr, a->dim, a->intdim));
	}
	return abstract0_of_opt_oct(man,
			opt_oct_add_epsilon_bin(man, a1->value, a2->value, epsilon));
}
