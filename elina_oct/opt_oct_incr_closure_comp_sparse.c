...
bool incremental_closure_comp_sparse(opt_oct_mat_t *oo, int dim, int v,
		bool is_int) {
	double *m = oo->mat;
	...
	int count = oo->nni;
	index1 = (unsigned short int *) calloc(2 * (2 * dim + 1),
			sizeof(unsigned short int));
	index2 = (unsigned short int *) calloc(2 * (2 * dim + 1),
			sizeof(unsigned short int));
	...
	if (cn != NULL) {
		...
		for (unsigned k = 0; k < comp_size; k++) {
			...
			//if(m[v1v2]!=INFINITY){
			m[v1v2] = min(m[v1v2],
					m[opt_matpos2(v1, k1)] + m[opt_matpos2(k1, v2)]);
			//}
			//else{
			...
			//}
			m[v1v2] = min(m[v1v2],
					m[opt_matpos2(v1, kk1)] + m[opt_matpos2(kk1, v2)]);
			//if(m[v2v1] != INFINITY){
			m[v2v1] = min(m[v2v1],
					m[opt_matpos2(v2, k1)] + m[opt_matpos2(k1, v1)]);
			//}
			//else{
			...
			//}
			m[v2v1] = min(m[v2v1],
					m[opt_matpos2(v2, kk1)] + m[opt_matpos2(kk1, v1)]);
