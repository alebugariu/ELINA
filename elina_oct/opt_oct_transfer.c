...
opt_oct_t* opt_oct_assign_linexpr(elina_manager_t* man, bool destructive,
		opt_oct_t* o, elina_dim_t d, elina_linexpr0_t* lexpr, opt_oct_t* dest) {
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man,
			ELINA_FUNID_ASSIGN_LINEXPR_ARRAY, 2 * (o->dim + 1 + 5));
	opt_uexpr u = opt_oct_uexpr_of_linexpr(pr, pr->tmp, lexpr, o->intdim,
			o->dim);
	opt_oct_mat_t* src;
	...
	if (!src)
		return opt_oct_set_mat(pr, o, NULL, NULL, destructive); /* empty */

	/* can / should we try to respect the closure */
	respect_closure = (src == o->closed) && (pr->funopt->algorithm >= 0)
			&& (!dest);

	if (!destructive)
		src = opt_hmat_copy(src, o->dim);
	...
	if (u.type == OPT_BINARY || u.type == OPT_OTHER)
		flag_incomplete;
	else if (num_incomplete || o->intdim)
		flag_incomplete;
	else if (!o->closed)
		flag_algo;
	else if (pr->conv)
	...
	if (respect_closure)
		return opt_oct_set_mat(pr, o, NULL, src, destructive);
	else
...
opt_oct_t* opt_oct_assign_linexpr_array(elina_manager_t* man, bool destructive,
		opt_oct_t* o, elina_dim_t* tdim, elina_linexpr0_t** lexpr, size_t size,
		opt_oct_t* dest) {
	if (size == 1)
		return opt_oct_assign_linexpr(man, destructive, o, tdim[0], lexpr[0],
				dest);

	opt_oct_internal_t* pr = opt_oct_init_from_manager(man,
			ELINA_FUNID_ASSIGN_LINEXPR_ARRAY, 2 * (o->dim + size + 5));
	...
	for (i = 0; i < size; i++) {
		...
		opt_uexpr u = opt_oct_uexpr_of_linexpr(pr, pr->tmp, lexpr[i], o->intdim,
				o->dim);
		...
		opt_hmat_assign(pr, u, dst, o->dim + size, o->dim + i,
				&respect_closure);
...
opt_oct_t* opt_oct_meet_lincons_array(elina_manager_t* man, bool destructive,
	...
	//fflush(stdout);
	opt_oct_internal_t* pr = opt_oct_init_from_manager(man,
			ELINA_FUNID_MEET_LINCONS_ARRAY, 2 * (o->dim + 8));
	if (!o->closed && !o->m)
		/* definitively empty */
		return opt_oct_set_mat(pr, o, NULL, NULL, destructive);
	else {
		...
		start_timing();
#endif
		bool res = opt_hmat_add_lincons(pr, oo, o->intdim, o->dim, array,
				&exact, &respect_closure);
		...
		if (res) {
			...
			if (num_incomplete || !exact)
				flag_incomplete;
			else if (pr->conv)
				flag_conv;
			opt_oct_t * r;
			if (respect_closure)
				r = opt_oct_set_mat(pr, o, NULL, oo, destructive);
			else
				r = opt_oct_set_mat(pr, o, oo, NULL, destructive);
...
opt_oct_t* opt_oct_meet_tcons_array(elina_manager_t* man, bool destructive,
	...
	return elina_generic_meet_intlinearize_tcons_array(man, destructive, o,
			array, ELINA_SCALAR_DOUBLE, ELINA_LINEXPR_INTLINEAR,
...
opt_oct_t* opt_oct_assign_texpr_array(elina_manager_t* man, bool destructive,
		...
		opt_oct_t* dest) {
	opt_oct_t *b = elina_generic_assign_texpr_array(man, destructive, o, tdim,
			texpr, size, dest);
	return b;
