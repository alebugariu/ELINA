...
double oct_is_unconstrained_time = 0;
...
bool opt_hmat_strong_closure(opt_oct_mat_t *oo, int dim) {
	...
	if (sparsity >= sparse_threshold) {
		...
		ind1 = (unsigned short int *) calloc(2 * (2 * dim + 1),
				sizeof(unsigned short int));
		ind2 = (unsigned short int *) calloc(2 * (2 * dim + 1),
				sizeof(unsigned short int));
		res = strong_closure_comp_sparse(oo, temp1, temp2, ind1, ind2, dim,
				flag);
		free(ind1);
		ind1 = NULL;
		free(ind2);
...
bool is_equal_half(opt_oct_mat_t *oo1, opt_oct_mat_t *oo2, int dim) {
	...
	if (!oo1->is_dense && !oo2->is_dense) {
		...
		if (!is_equal_array_comp_list(oo1->acl, oo2->acl, dim)) {
			 ...
			 ****/
			array_comp_list_t * acl = intersection_array_comp_list(oo1->acl,
					oo2->acl, dim);
			/****
			 Check 1
			 ****/
			comp_list_t *cl1 = oo1->acl->head;
			unsigned short int * arr_map = create_array_map(acl, dim);
			while (cl1 != NULL) {
...
void meet_half(opt_oct_mat_t *oo, opt_oct_mat_t *oo1, opt_oct_mat_t *oo2,
		int dim, bool destructive) {
#if defined(TIMING)
	start_timing();
#endif
	double *m = oo->mat;
	...
	if (!oo1->is_dense && !oo2->is_dense) {
		...
		/*****
		 Step 1
		 ******/
		array_comp_list_t * acl = union_array_comp_list(oo1->acl, oo2->acl,
				dim);
		oo->is_dense = false;
		if (!destructive) {
			oo->ti = false;
		...
		comp_list_t * cl = acl->head;
		unsigned short int * arr_map1 = create_array_map(oo1->acl, dim);
		unsigned short int * arr_map2 = create_array_map(oo2->acl, dim);
		while (cl != NULL) {
			unsigned short int comp_size = cl->size;
			unsigned short int * ca = to_sorted_array(cl, dim);
			/*****
...
void forget_array_half(opt_oct_mat_t *oo, elina_dim_t *arr, int dim,
		int arr_dim, bool project) {
...
void join_half(opt_oct_mat_t *oo, opt_oct_mat_t *oo1, opt_oct_mat_t *oo2,
		int dim, bool destructive) {
#if defined(TIMING)
	start_timing();
#endif
...
void opt_hmat_addrem_dimensions(opt_oct_mat_t * dst_mat, opt_oct_mat_t* src_mat,
		elina_dim_t* pos, int nb_pos, int mult, int dim, bool add) {
#if defined(TIMING)
	start_timing();
#endif
	int i, j, new_j, org_j;
	...
	double * src = src_mat->mat;
	unsigned short int * map = (unsigned short int *) calloc(dim + 1,
			sizeof(unsigned short int));
			...
			while (cl != NULL) {
				...
				for (unsigned short int i = 0; i < comp_size; i++) {
					...
					for (unsigned short int j = 0; j < comp_size; j++) {
						...
						dst[opt_matpos2(2 * ni, 2 * nj)] = src[opt_matpos2(
								2 * i1, 2 * j1)];
						dst[opt_matpos2(2 * ni, 2 * nj + 1)] = src[opt_matpos2(
								2 * i1, 2 * j1 + 1)];
						dst[opt_matpos2(2 * ni + 1, 2 * nj)] = src[opt_matpos2(
								2 * i1 + 1, 2 * j1)];
						dst[opt_matpos2(2 * ni + 1, 2 * nj + 1)] =
								src[opt_matpos2(2 * i1 + 1, 2 * j1 + 1)];
					}
				}
				free(ca);
				cl = cl->next;
...
void opt_hmat_permute(opt_oct_mat_t* dest_mat, opt_oct_mat_t* src_mat,
		int dst_dim, int src_dim, elina_dim_t* permutation) {
#if defined(TIMING)
	start_timing();
#endif
...
void widening_half(opt_oct_mat_t *oo, opt_oct_mat_t *oo1, opt_oct_mat_t *oo2,
		int dim) {
#if defined(TIMING)
	start_timing();
#endif
...
void widening_thresholds_half(opt_oct_mat_t *oo, opt_oct_mat_t *oo1,
		opt_oct_mat_t *oo2, double *thresholds, int nb, int dim) {
#if defined(TIMING)
	start_timing();
#endif
	double *m = oo->mat;
...
void narrowing_half(opt_oct_mat_t *oo, opt_oct_mat_t *oo1, opt_oct_mat_t *oo2,
		int dim) {
#if defined(TIMING)
	start_timing();
#endif
...
#define COEFF(c,i)                                                      \
  ...
  if (c.discr!=ELINA_COEFF_SCALAR || !is_integer(dst[i])) u.is_int = 0;
	...
	case ELINA_LINEXPR_SPARSE:
		...
		break;
	default:
		break;/********TODO: handle arg_assert arg_assert(0,return u;);*****/
	}
	return u;
}

void opt_bounds_mul(double a_inf, double a_sup, double b_inf, double b_sup,
...
bool opt_hmat_add_lincons(opt_oct_internal_t* pr, opt_oct_mat_t* oo, int intdim,
		int dim, elina_lincons0_array_t* ar, bool* exact, bool* respect_closure) {
	double *m = oo->mat;
	int i, j, k, ui, uj;
	...
	*exact = 1;
	...
	for (i = 0; i < ar->size; i++) {
		...
		u = opt_oct_uexpr_of_linexpr(pr, pr->tmp, ar->p[i].linexpr0, intdim,
				dim);
		...
		case OPT_UNARY:
			...
			if (m[opt_matpos(ui, ui ^ 1)] == INFINITY) {
				...
				m[opt_matpos(ui, ui ^ 1)] = min(m[opt_matpos(ui, ui ^ 1)],
						pr->tmp[1]);
			}
			...
			if (c == ELINA_CONS_EQ) {
				if (m[opt_matpos(ui ^ 1, ui)] == INFINITY) {
					...
					m[opt_matpos(ui ^ 1, ui)] = min(m[opt_matpos(ui ^ 1, ui)],
							pr->tmp[0]);
				}
			}
		...
		case OPT_BINARY:
			...
			if (m[opt_matpos2(uj, ui ^ 1)] == INFINITY) {
				...
				m[opt_matpos2(uj, ui ^ 1)] = min(m[opt_matpos2(uj, ui ^ 1)],
						pr->tmp[1]);
			}
			...
			if (c == ELINA_CONS_EQ) {
				if (m[opt_matpos2(uj ^ 1, ui)] == INFINITY) {
					...
					m[opt_matpos2(uj ^ 1, ui)] = min(m[opt_matpos2(uj ^ 1, ui)],
							pr->tmp[0]);
				}
			}
		...
		case OPT_OTHER: {
			...
			double * Cb_map = (double *) calloc(dim, sizeof(double));
			double * cb_map = (double *) calloc(dim, sizeof(double));
			...
			*respect_closure = false; /* do not respect closure */
			...
			if (Cb == INFINITY)
				...
				if ((pr->tmp[2 * Cj1 + 3] == -1) && (pr->tmp[2 * Cj1 + 2] == 1))
					uj = 2 * Cj1;
				...
				for (k = 0; k < dim; k++) {
					...
					if (pr->tmp[2 * k + 2] <= -1) {
						...
						if (m[opt_matpos(2 * k + 1, 2 * k)] != INFINITY) {
						...
						if (m[opt_matpos(2 * k, 2 * k + 1)] != INFINITY) {
							...
							if (m[opt_matpos2(2 * k + 1, uj)] == INFINITY) {
								...
								m[opt_matpos2(2 * k + 1, uj)] = min(
										m[opt_matpos2(2 * k + 1, uj)], tmpb);
							}
						}
					} else if (pr->tmp[2 * k + 2] == -pr->tmp[2 * k + 3]) {

						double res = Cb - Cb_map[k];
						if ((pr->tmp[2 * k + 2] < 0)
								&& m[opt_matpos(2 * k, 2 * k + 1)]
										!= INFINITY) {
							res = res
									+ (1 + pr->tmp[2 * k + 2])
											* m[opt_matpos(2 * k, 2 * k + 1)];
							res = res / 2;
							if (m[opt_matpos2(2 * k, uj)] == INFINITY) {
								m[opt_matpos2(2 * k, uj)] = res;
								count++;
							} else {
								m[opt_matpos2(2 * k, uj)] = min(
										m[opt_matpos2(2 * k, uj)], res);
							}
						} else if ((pr->tmp[2 * k + 3] < 0)
								&& m[opt_matpos(2 * k + 1, 2 * k)]
										!= INFINITY) {
							double res = Cb - Cb_map[k];
							res = res
									+ (1 + pr->tmp[2 * k + 3])
				...
				if ((pr->tmp[2 * Cj1 + 3] == -1) && (pr->tmp[2 * Cj1 + 2] == 1))
					...
					goto Cbrk;
				if ((pr->tmp[2 * Cj2 + 3] == -1) && (pr->tmp[2 * Cj2 + 2] == 1))
					uj = 2 * Cj2;
 ...
 */
void opt_hmat_assign(opt_oct_internal_t* pr, opt_uexpr u, opt_oct_mat_t* oo,
		size_t dim, size_t d, bool* respect_closure) {
	size_t i, k;
	double *m = oo->mat;
	bool (*incr_closure)(opt_oct_mat_t *, ...);
	double size = 2 * dim * (dim + 1);
	...
	if (u.type == OPT_ZERO) {
				...
				while (cl != NULL) {
					...
					while (c != NULL) {
						...
						if (pr->tmp[2] != INFINITY) {
							m[opt_matpos2(2 * d, 2 * k)] = pr->tmp[2]
									+ pr->tmp[0];
							m[opt_matpos2(2 * d + 1, 2 * k)] = pr->tmp[2]
									+ pr->tmp[1];
							count += 2;
						} else {
						...
						if (pr->tmp[2] != INFINITY) {
							m[opt_matpos2(2 * d, 2 * k + 1)] = pr->tmp[2]
									+ pr->tmp[0];
							m[opt_matpos2(2 * d + 1, 2 * k + 1)] = pr->tmp[2]
									+ pr->tmp[1];
							count += 2;
						} else {
		...
		if (!oo->is_dense) {
			...
			if (cl != NULL) {
				...
				while (c != NULL) {
					...
					m[opt_matpos2(2 * d, 2 * k)] = m[opt_matpos2(2 * d + 1,
							2 * k)] + pr->tmp[0];
					m[opt_matpos2(2 * d + 1, 2 * k)] = pr->tmp[2] + pr->tmp[1];
					pr->tmp[2] = m[opt_matpos2(2 * d, 2 * k + 1)];
					m[opt_matpos2(2 * d, 2 * k + 1)] = m[opt_matpos2(2 * d + 1,
							2 * k + 1)] + pr->tmp[0];
					m[opt_matpos2(2 * d + 1, 2 * k + 1)] = pr->tmp[2]
							+ pr->tmp[1];
					//}
				...
				m[opt_matpos(2 * d + 1, 2 * d)] =
						m[opt_matpos(2 * d, 2 * d + 1)] + pr->tmp[1];
			...
			for (k = 0; k < 2 * d; k++) {
				...
				m[opt_matpos(2 * d, k)] = m[opt_matpos(2 * d + 1, k)]
						+ pr->tmp[0];
				m[opt_matpos(2 * d + 1, k)] = pr->tmp[2] + pr->tmp[1];
			}
			for (k = 2 * d + 2; k < 2 * dim; k++) {
				pr->tmp[2] = m[opt_matpos(k, 2 * d)];
				m[opt_matpos(k, 2 * d)] = m[opt_matpos(k, 2 * d + 1)]
						+ pr->tmp[1];
				m[opt_matpos(k, 2 * d + 1)] = pr->tmp[2] + pr->tmp[0];
			}
			count += 4 * dim - 4;
			...
			pr->tmp[1] = 2 * pr->tmp[1];
			pr->tmp[2] = m[opt_matpos(2 * d + 1, 2 * d)];
			m[opt_matpos(2 * d + 1, 2 * d)] = m[opt_matpos(2 * d, 2 * d + 1)]
					+ pr->tmp[1];
			m[opt_matpos(2 * d, 2 * d + 1)] = pr->tmp[2] + pr->tmp[0];
			count += 2;
		}
		...
		*respect_closure = false; /* does not respect closure */
		...
		for (i = 0; i < dim; i++) {
			...
			opt_bounds_mul(m[opt_matpos(2 * i, 2 * i + 1)],
					m[opt_matpos(2 * i + 1, 2 * i)], pr->tmp[2 * i + 2],
					pr->tmp[2 * i + 3], &tmpa, &tmpb);
			if (tmpa == INFINITY) {
				cinf++;
				ci = i;
			} else
		...
		if (!Cinf) {
			...
			if (Ci != d) {
				if ((pr->tmp[2 * i + 3] == 1) && (pr->tmp[2 * i + 2] == -1)) {
					if (!oo->is_dense) {
						/*****
